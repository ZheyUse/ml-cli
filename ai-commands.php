...
// Build PowerShell here-string content for server script
// Important: @ must be on its own line, @" starts here-string, content on next line, "@ ends on its own line
function buildServerHereString(): string
{
    return '$serverScript = [System.IO.Path]::GetTempFileName() + ".ps1"' . PHP_EOL .
        '$serverBody = @"' . PHP_EOL .
        '$Host.UI.RawUI.WindowTitle = "ml --ai fcc-server (bg)"' . PHP_EOL .
        'Set-Location ' . psSingleQuote(FCC_INSTALL_DIR) . PHP_EOL .
        'uv run ' . FCC_PROJECT_FLAG . ' fcc-server' . PHP_EOL .
        '"@' . PHP_EOL .
